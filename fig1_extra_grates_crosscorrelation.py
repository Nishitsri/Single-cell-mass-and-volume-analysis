## To measure cross-correlation between mass and volume
...
# CROSS CORRELATION
delay_vec = [1, 2, 3, 4, 5, 6, 7, 8]
binned_gratemass_X_vs_gratevol_Y_crosscorr = []
par_cross_corr = []
for delay in delay_vec:
    gratemass_vs_gratevol_crosscorr, corr_gratemass_vs_gratevol = cc.scatter_plot_vectors_delayed(cells_grate_mass_as_bm, cells_grate_vol_as_bm, delay)
    par_cross_corr.append(linregress(gratemass_vs_gratevol_crosscorr[0], gratemass_vs_gratevol_crosscorr[1]))
    binned_gratemass_X_vs_gratevol_Y_crosscorr.append(cc.binningdata_bins_mean(gratemass_vs_gratevol_crosscorr[0], gratemass_vs_gratevol_crosscorr[1], window_size))
...
rvalue_vs_delay = [ par_cross_corr[i].rvalue for i in range(len(delay_vec)) ]
slope_vs_delay = [ par_cross_corr[i].slope for i in range(len(delay_vec)) ]
ax.plot(delay_vec, rvalue_vs_delay, 'o',
...
fig.savefig('Hela_growth_rate_lag_cross_correlation_grmassX_grvolY_derstep%.2F.pdf' % tau, bbox_inches='tight')  
plt.close(fig) 

# GROWTH RATE CORRELATION BINNED
for n in range(len(delay_vec)):
    width = 2
    height = width/1.618
    fig, ax = plt.subplots(figsize=(width, height), dpi=100)
    fig.subplots_adjust(left=.15, bottom=.16, right=.99, top=.99)  
    ...
    ax.plot(binned_gratemass_X_vs_gratevol_Y_crosscorr[n][0], binned_gratemass_X_vs_gratevol_Y_crosscorr[n][1], 'o',
    ...
    fig.savefig('Hela_growth_rate_lag_cross_correlation_%.2fhbinned_plot_grmassX_grvolY_derstep%.2F.pdf' % (delay_vec[n], tau), bbox_inches='tight')  
    plt.close(fig) 
